          ...
          @foreach ($domains as $domain)
            @if ($domain->prices_count > 0)
              <tr>
                ...
                <td class="text-center bg-light"><img src="{{ Storage::url($domain->prices()->orderBy('price')->first()->registrar->logo) }}" height="20" alt="registrar_logo"> <a href="{{ route('registrars.show', ['slug' => $domain->prices()->orderBy('price')->first()->registrar->slug]) }}" class="text-dark">{{ $domain->prices()->orderBy('price')->first()->registrar->name }}</a></td>
